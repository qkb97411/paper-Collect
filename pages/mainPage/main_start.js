...
Page({
  ...
  switchTab(e) {
    ...
    if (tab === 0) {
      wx.navigateTo({
        ...
        fail: (err) => {
          ...
          wx.navigateBack(); // 兜底返回
        }
      });
    }
    if (tab === 1) {
      wx.navigateTo({
        url: "/subpkg/Polaroid/PolaroidAllList/PolaroidAllList",
        fail: (err) => {
          console.error("跳转 Polaroid 分包失败：", err);
          wx.showToast({ title: "页面加载失败", icon: "none" });
        }
      });
    }
    if (tab === 2) {
      wx.navigateTo({
        url: "/subpkg/Mine/MyHome/MyHome",
        fail: (err) => {
          console.error("跳转 我的 分包失败：", err);
          wx.showToast({ title: "页面加载失败", icon: "none" });
        }
      });
    }
